...
def swine_align(player_score, opponent_score):
    ...
    if player_score<10 or opponent_score<10:
        ...
        for x in range(2,min_score+1):
            if player_score%x==0 and opponent_score%x==0 and x>=10:
                return True
            elif x==min_score:
                return False
    # END PROBLEM 4a
...
def announce_lead_changes(last_leader=None):
    ...
    #注意这个函数在调用的时候返回了say函数,所以才能在下面的示例中传入两个参数
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    #注意给ma中传的参数目的是为给roll_dice函数传入参数
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score+free_bacon(opponent_score),opponent_score)==True:
        return 0
    elif bacon_strategy(score, opponent_score, cutoff, num_rolls)==0 and extra_turn(score+free_bacon(opponent_score),opponent_score)==False:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
